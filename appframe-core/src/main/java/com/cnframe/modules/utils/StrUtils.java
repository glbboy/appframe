...
public class StrUtils {  
    ...
    /** 
     * 根据实体名获取tableName，即把大写字母变成_加小写字母 
     * @param entityName 
     * @return 
     * @author lqy 
     */  
    public static String clazzNameToTableName(String clazzName){  
        if(StrUtils.isEmpty(clazzName)){  
            return null;  
        }  
        String nameTrim = clazzName.trim();  
        int length = nameTrim.length();  
        StringBuffer tableName = new StringBuffer("");  
        if(length > 0){  
            for (int i = 0; i < length; i++) {  
                char c = nameTrim.charAt(i);  
                if(!"".equals(c)){  
//                    if(RegUtils.isUpperCase(String.valueOf(c)) && i > 0 ){//i!=0排除第一个大写字母，不加下划线  
//                        tableName.append(StrUtils.replaceUpperCase(String.valueOf(c)));  
//                    }else{  
//                        tableName.append(String.valueOf(c).toLowerCase());  
//                    }  
                }  
            }  
        }  
        //System.out.println("=====tableName.toString()="+tableName.toString());  
