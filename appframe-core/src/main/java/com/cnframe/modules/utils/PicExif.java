...
public class PicExif {
	...
	public static ArrayList<PhotoInfoDetail> getMetaData(String filePath,Long imgId){
		File file = new File(filePath);
		ArrayList<PhotoInfoDetail> photoDetailList = new ArrayList<PhotoInfoDetail>();
		try {
            Metadata metadata = ImageMetadataReader.readMetadata(file);
            print(metadata, "Using ImageMetadataReader");
            for (Directory directory : metadata.getDirectories()) {
                for (Tag tag : directory.getTags()) {
                	PhotoInfoDetail photoInfoDetail = new PhotoInfoDetail();
                	photoInfoDetail.setImgId(imgId);
                	photoInfoDetail.setTagMain(tag.getDirectoryName());
                	photoInfoDetail.setTagCode(tag.getTagName());
                	photoInfoDetail.setTagValue(tag.getDescription());
                	photoDetailList.add(photoInfoDetail);
                }
    ...
    {
		getMetaData("F:/图片/婚纱照1/CIMG0804.JPG",11l);
		//File file = new File("Tests/Data/withIptcExifGps.jpg");
		//File file = new File("F:/图片/婚纱照1/CIMG0804.JPG");
